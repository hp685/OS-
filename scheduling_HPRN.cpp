...
vector<long long> U;
vector<process> P,CP,CPZ; //CP is a copy of P where state is unchanged.
...
void schedule()
{
  ...
  while(flag)
      ...
      if(current.C == 0)
	{
	  ...
	  //  float min = 0.0;
	      ...
	      for(int i = 0 ; i < P.size(); i++)
		{
		  ...
		  if(counter == 0) // NO Ready states available
		    for(int i = 0; i < CP.size(); i++)
		      {
			if(CP[i].state == "blocked")
			  tracker = i;
      ...
      else
	{
	  ...
	  int tracker = 0,counter = 0;float min = 0.0;
	  sort(CP.begin(),CP.end(),sortbyPR);
	  for(int i = 0; i < CP.size(); i++)
	    {
